...
export function getSharedYtDlpConfig(url, playerClients = "android,ios,tv") {
  ...
  if (isYouTube && playerClients) {
    baseFlags.extractorArgs = `youtube:player_client=${playerClients}`;
  }

  return { baseFlags, cookiesPath, proxyUrl, isYouTube };
}
...
export async function runYtDlp(url, flags = {}) {
  ...
  const clientStrategies = isYouTube
    ? ["android,ios,tv", "android_vr,tv_embedded,web_embedded", "web_embedded,mweb", ""]
    : [""];

  let lastError;

  for (const clientTier of clientStrategies) {
    const { baseFlags } = getSharedYtDlpConfig(url, clientTier);
    try {
      ...
      lastError = err;
      console.warn(`[runYtDlp] Attempt with clientTier "${clientTier || 'default'}" failed:`, err?.message || err);
    }
  }
