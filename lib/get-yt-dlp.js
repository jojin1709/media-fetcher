...
export async function getYtDlpPath() {
  ...
  // 1. Check pre-bundled binary in node_modules/youtube-dl-exec/bin (installed during postinstall build step)
  const candidatePaths = [
    ...
    path.join(os.tmpdir(), primaryBin),
  ];

  for (const p of candidatePaths) {
    if (fs.existsSync(p)) {
      console.log(`[getYtDlpPath] Using pre-bundled binary at ${p}`);
      if (!isWin) {
        try {
  ...
  // 2. Download standalone binary to tmp directory if not found
  const tmpPath = path.join(os.tmpdir(), primaryBin);
  console.log(`[getYtDlpPath] Pre-bundled binary not found. Downloading standalone binary to ${tmpPath}...`);
  ...
  console.log(`[getYtDlpPath] Successfully downloaded binary to ${tmpPath}`);
  return tmpPath;
}
